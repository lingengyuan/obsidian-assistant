...
def suggest_merges_command(analyzer: SimilarityAnalyzer, args):
    ...
    for i, (note1, note2, similarity, reasons) in enumerate(suggestions[:args.limit], 1):
...
def main():
    ...
    find_parser = subparsers.add_parser('find', help='Find notes similar to a specific note')
    find_parser.add_argument('note_name', help='Note name to find similar notes for')
    find_parser.add_argument('--limit', type=int, default=5, help='Number of results (default: 5)')
    
    # duplicates 命令 - 查找可能重复的笔记
    dup_parser = subparsers.add_parser('duplicates', help='Find potential duplicate notes')
    dup_parser.add_argument('--threshold', type=float, help='Similarity threshold (default: 0.7)')
    dup_parser.add_argument('--limit', type=int, default=20, help='Number of results (default: 20)')
    
    # unlinked 命令 - 查找相关但未链接的笔记
    unlink_parser = subparsers.add_parser('unlinked', help='Find related but unlinked notes')
    unlink_parser.add_argument('--limit', type=int, default=20, help='Number of results (default: 20)')
    
    # merge 命令 - 建议合并的笔记
    merge_parser = subparsers.add_parser('merge', help='Suggest notes to merge')
    merge_parser.add_argument('--threshold', type=float, help='Similarity threshold (default: 0.6)')
    merge_parser.add_argument('--limit', type=int, default=10, help='Number of results (default: 10)')
    
    # all 命令 - 分析所有相似笔记对
    all_parser = subparsers.add_parser('all', help='Analyze all similar note pairs')
    all_parser.add_argument('--threshold', type=float, help='Minimum similarity threshold (default: 0.3)')
    all_parser.add_argument('--limit', type=int, default=20, help='Number of results to display (default: 20)')
