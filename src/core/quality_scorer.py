...
class QualityScorer:
    ...
    def get_statistics(self, scores: Dict[str, QualityScore]) -> Dict:
        ...
        # 需要改进的笔记（低于70分）
        needs_improvement = [s for s in scores.values() if s.percentage < 70]
        needs_improvement.sort(key=lambda x: x.percentage)

        # 优质笔记（90分以上）
        excellent_notes = [s for s in scores.values() if s.percentage >= 90]
    ...
    def get_top_issues(
        self, scores: Dict[str, QualityScore], limit: int = 10
    ) -> List[Tuple[str, List[str]]]:
        """获取最需要改进的笔记及其问题"""
        scored_notes = [
            (s.note_name, s.percentage, s.issues) for s in scores.values() if s.issues
        ]
...
def generate_quality_report(scores: Dict[str, QualityScore], stats: Dict) -> str:
    ...
    if stats["excellent_notes"]:
        ...
        for i, score in enumerate(stats["excellent_notes"][:10], 1):
            lines.append(
                f"{i}. **{score.note_name}** - {score.percentage:.1f}分 ({score.grade})"
            )
