...
def display_search_results(results: List[Note], title: str):
    ...
    for i, note in enumerate(results, 1):
        print(f"  {i}. {note.name}")
        print(
            f"     Words: {note.word_count} | Links: {note.total_links} (↗{len(note.outgoing_links)} ↘{len(note.incoming_links)})"
        )
        if note.tags:
...
def search_command(analyzer: ObsidianAnalyzer, args):
    ...
    results = analyzer.search_notes(
        query=query, tags=tags, min_links=min_links, max_links=max_links
    )
...
def list_hubs_command(analyzer: ObsidianAnalyzer, args):
    """列出知识枢纽"""
    hubs = sorted(
        analyzer.notes.values(), key=lambda x: len(x.outgoing_links), reverse=True
    )
...
def list_popular_command(analyzer: ObsidianAnalyzer, args):
    """列出热门笔记"""
    popular = sorted(
        analyzer.notes.values(), key=lambda x: len(x.incoming_links), reverse=True
    )
...
def main():
    ...
    search_parser.add_argument(
        "query", nargs="?", help="Search query (keyword in name or content)"
    )
    search_parser.add_argument(
        "--tags", type=str, help="Filter by tags (comma-separated)"
    )
