...
def list_by_score(scores: dict, args):
    ...
    for i, score in enumerate(sorted_scores, 1):
        ...
        print(f"     分数: {score.percentage:.1f}  |  字数:{score.word_count_score:.0f}  链接:{score.link_score:.0f}  标签:{score.tag_score:.0f}  新鲜:{score.freshness_score:.0f}")
        print()
...
def show_excellent(stats: dict, limit: int = 10):
    ...
    for i, score in enumerate(excellent, 1):
        print(f"{i}. {score.note_name} - {score.percentage:.1f}分 ({score.grade})")
        print(f"   字数:{score.word_count_score:.0f}  链接:{score.link_score:.0f}  标签:{score.tag_score:.0f}  新鲜:{score.freshness_score:.0f}")
        print()
...
def check_note(scores: dict, note_name: str):
    ...
    if len(matches) == 1:
        ...
        for i, match in enumerate(matches[:10], 1):
            print(f"{i}. {match} - {scores[match].percentage:.1f}分 ({scores[match].grade})")
...
def main():
    ...
    list_parser.add_argument('--grade', type=str, choices=['A', 'B', 'C', 'D', 'F'], help='Filter by grade')
    list_parser.add_argument('--ascending', action='store_true', help='Sort ascending (low to high)')
    ...
    worst_parser.add_argument('--limit', type=int, default=10, help='Number of notes to show')
    
    # best 命令
    best_parser = subparsers.add_parser('best', help='Show best notes')
    best_parser.add_argument('--limit', type=int, default=10, help='Number of notes to show')
