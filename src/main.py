...
def analyze_single_vault(
    vault_path: Path, exclude_folders: list, exclude_notes: list, args
):
    """分析单个 vault"""
    print("=" * 60)
    ...
    try:
        ...
        if not args.no_report:
            ...
            if os.getenv("ENABLE_QUALITY_SCORING", "true").lower() == "true":
                ...
                # 保存质量报告
                quality_report_path = Path(output_dir) / datetime.now().strftime(
                    "quality-report-%Y-%m-%d.md"
                )
                ...
                print(
                    f"  Grade A notes:     {quality_stats['grade_distribution']['A']}"
                )
            ...
            if (
                args.export
                or os.getenv("EXPORT_JSON", "true").lower() == "true"
                or os.getenv("EXPORT_CSV", "true").lower() == "true"
            ):
                print()
...
def analyze_multi_vaults(
    vault_paths: list, exclude_folders: list, exclude_notes: list, args
):
    """分析多个 vaults"""
    print("=" * 60)
    ...
    try:
        ...
        for vault_name, breakdown in combined_stats["vault_breakdown"].items():
            print(f"  • {vault_name}:")
            print(
                f"    Notes: {breakdown['notes']} | Words: {breakdown['words']:,} | Orphans: {breakdown['orphans']}"
            )
        print()
            ...
            for vault_name, analyzer in multi_analyzer.analyzers.items():
                ...
                if args.export:
                    exporter = DataExporter(
                        stats, analyzer.notes, str(analyzer.vault_path)
                    )
...
def main():
    ...
    parser.add_argument("--vault", type=str, help="Obsidian vault 路径（覆盖环境变量）")
    parser.add_argument("--multi-vault", type=str, help="多个 vault 路径（逗号分隔）")
    parser.add_argument("--output", type=str, help="报告输出目录（覆盖环境变量）")
    parser.add_argument("--no-report", action="store_true", help="只分析不生成报告")
    parser.add_argument("--export", action="store_true", help="导出 JSON 和 CSV 数据")
